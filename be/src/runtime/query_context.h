...
public:
    ...
    // Query-global CPU/exec runtime counter shared by every PipelineFragmentContext of
    // this query, so all of its fragments accumulate into one number. The closed-system
    // scheduler does not rank on it; it is kept as a per-query CPU total for profiling.
