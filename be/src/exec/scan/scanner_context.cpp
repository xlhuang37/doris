...
Status ScannerContext::init() {
    ...
    if (auto* task_executor_scheduler =
        ...
        // Level this scan task by the owning query's unified CPU runtime (shared
        // with the pipeline MLFQ). The query context strictly outlives its
        ...
        if (auto exec_ctx = task_exec_ctx()) {
            query_runtime = exec_ctx->query_runtime_counter();
        }
