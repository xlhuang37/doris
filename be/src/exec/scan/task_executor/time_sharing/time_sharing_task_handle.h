...
#include "exec/scan/task_executor/time_sharing/split_concurrency_controller.h"
...
private:
    ...
    // Owning query's unified CPU runtime counter (nullable). When set, this task's
    // splits are leveled by the query runtime (shared with the pipeline MLFQ) and
