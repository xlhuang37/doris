...
private:
    ...
    TaskScheduler(int core_num, std::string name, std::shared_ptr<CgroupCpuCtl> cgroup_cpu_ctl,
                  MultiCoreTaskQueue::Mode queue_mode = MultiCoreTaskQueue::Mode::CLOSED)
            : _name(std::move(name)),
...
public:
    ...
    // remote/AI functions, recursive CTEs) and cannot honor the "re-check the slot every
    // execution slice" invariant the closed-system partition relies on, so its queue runs
    // in the degenerate general-only mode (one plain shared lock-free queue). These
    // workers are outside the partition, so keep this pool small when profiling: a query
    // with blockable tasks gets parallelism here that its slot did not grant.
