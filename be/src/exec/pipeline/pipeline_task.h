...
public:
    ...
    // Executed CPU time is charged to the owning query's global counter (shared across
    // all of the query's fragments, instances and pipeline tasks). The closed-system
    // scheduler makes no decision from it; it is a per-query CPU total for profiling.
